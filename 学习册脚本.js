...
function getEnglishVoice(voices) {
  return voices.find((voice) => /google|microsoft|samantha|alex/i.test(voice.name) && /^en(-|_)/i.test(voice.lang))
...
function speakWithSystemVoice(word) {
  ...
  const voice = preferredVoice || getEnglishVoice(synth.getVoices());
...
function speak(word) {
  ...
  if (speakWithSystemVoice(word)) return;

  speakWithNetworkVoice(word);
}
...
if ('speechSynthesis' in window) {
  ...
  preferredVoice = getEnglishVoice(voices);
  window.speechSynthesis.addEventListener('voiceschanged', () => {
    preferredVoice = getEnglishVoice(window.speechSynthesis.getVoices());
  }, { once: true });
}
